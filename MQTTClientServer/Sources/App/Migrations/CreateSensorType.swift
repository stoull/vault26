//
//  CreateSensorType.swift
//  MQTTClientServer
//
//  空库初始化：创建 sensor_type
//
...
struct CreateSensorType: AsyncMigration {
    var name: String { "CreateSensorType" }

    func prepare(on database: Database) async throws {
        try await database.schema(SensorType.schema)
    ...
    func revert(on database: Database) async throws {
        try await database.schema(SensorType.schema).delete()
    }
